/*
 * mii.c — libmii: Read Mii data from the Wii's NAND filesystem.
 *
 * Original library by libmii authors.  Reads the RNOD-format
 * FaceLib database and parses all 100 Mii slots.
 */
...
#define FACELIB_PATH "/shared2/menu/FaceLib/RFL_DB.dat"
#define MII_DB_SIZE  ((size_t)(MII_MAX * MII_SIZE + MII_HEADER))
...
Mii *loadMiis_Wii(void) {
    ...
    if (ISFS_Initialize() < 0) {
        printf("libmii: ISFS_Initialize failed\n");
        return NULL;
    }

    fd = ISFS_Open(FACELIB_PATH, ISFS_OPEN_READ);
    if (fd < 0) {
        printf("libmii: cannot open %s (err %d)\n", FACELIB_PATH, fd);
        return NULL;
    }

    /* Buffer must be 32-byte aligned for IOS DMA. */
    buffer = (char *)memalign(32, MII_DB_SIZE);
    if (!buffer) {
        ISFS_Close(fd);
        return NULL;
    }

    if (ISFS_Read(fd, buffer, (u32)MII_DB_SIZE) < 0) {
        printf("libmii: read failed\n");
        free(buffer);
        ISFS_Close(fd);
        return NULL;
    }
    ...
    ISFS_Close(fd);
    return loadMiis(buffer);
#else
    fprintf(stderr, "[libmii] loadMiis_Wii stub on host\n");
    return NULL;
#endif
}

Mii loadMii_from_raw(int start, char *data) {
    Mii mii;
    int c, d;
    char tempChar;
    ...
    if (data[start + 0x24] & 64) mii.eyebrowRotation += 1;
    if (data[start + 0x24] & 128) mii.eyebrowRotation += 2;
    if (data[start + 0x25] & 1) mii.eyebrowRotation += 4;
    if (data[start + 0x25] & 2) mii.eyebrowRotation += 8;
    ...
    mii.noseSize = 0;
    if (data[start + 0x2C] & 16) mii.noseSize += 1;
    if (data[start + 0x2C] & 32) mii.noseSize += 2;
    if (data[start + 0x2C] & 64) mii.noseSize += 4;
    if (data[start + 0x2C] & 128) mii.noseSize += 8;

    mii.noseVertPos = 0;
    if (data[start + 0x2D] & 1) mii.noseVertPos += 1;
    if (data[start + 0x2D] & 2) mii.noseVertPos += 2;
    if (data[start + 0x2D] & 4) mii.noseVertPos += 4;
    if (data[start + 0x2D] & 8) mii.noseVertPos += 8;
    if (data[start + 0x2D] & 16) mii.noseVertPos += 16;
    ...
    mii.glassesSize = 0;
    if (data[start + 0x30] & 128) mii.glassesSize += 1;
    if (data[start + 0x31] & 1) mii.glassesSize += 2;
    if (data[start + 0x31] & 2) mii.glassesSize += 4;
...
Mii *loadMiis(char *data) {
    ...
    if (data[0] == 'R' && data[1] == 'N' &&
        ...
        for (n = 0; n < MII_MAX; n++) {
            ...
            miis[n] = loadMii_from_raw(start, data);
        }
    } else {
