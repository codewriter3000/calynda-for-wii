    ...
    if (selected->selection.kind == CODEGEN_SELECTION_DIRECT) {
        ...
        case CODEGEN_DIRECT_ABI_ARG_MOVE: {
            ...
            if (instruction->as.incoming_arg.argument_index < 6) {
                ...
                mc_format_incoming_arg_stack_operand(instruction->as.incoming_arg.argument_index - 6,
                                                     &source);
            }
        ...
        case CODEGEN_DIRECT_ABI_OUTGOING_ARG: {
            ...
            if (call_selected->selection.kind == CODEGEN_SELECTION_DIRECT &&
                ...
                if (instruction->as.outgoing_arg.argument_index < 6) {
                    ...
                    destination = NULL;
                    mc_format_outgoing_arg_stack_operand(instruction->as.outgoing_arg.argument_index - 6,
                                                         &destination);
                }
        ...
        case CODEGEN_DIRECT_STORE_SLOT: {
            ...
            if (!mc_format_slot_operand(codegen_unit, instruction->as.store_slot.slot_index, &destination) ||
                !mc_format_operand(lir_unit, codegen_unit, instruction->as.store_slot.value, &source)) {
                free(destination);
                free(source);
                return false;
            }
        ...
        case CODEGEN_DIRECT_STORE_GLOBAL: {
            ...
            if (!destination ||
                !mc_format_operand(lir_unit, codegen_unit, instruction->as.store_global.value, &source)) {
                free(destination);
                free(source);
                return false;
            }
