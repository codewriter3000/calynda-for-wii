/*
 * test_solid_style.c — Unit tests for the CSS-like style property system.
 *
 * Tests:
...
static int style_lookup(const char *name)
{
    ...
    if (name && solid_style_lookup_name(name, &out)) return (int)out;
    return -1;
}
...
{
    SolidStyleMap smap;
    solid_style_map_init(&smap);
    assert(smap.count == 0);
    solid_style_map_free(&smap);
...
static void test_style_set_get(void)
{
    ...
    SolidStyleValue v = solid_style_int(320);
    solid_style_map_set(&smap, SOLID_STYLE_PROP_WIDTH, v);

    const SolidStyleValue *got = solid_style_map_get(&smap, SOLID_STYLE_PROP_WIDTH);
    assert(got != NULL);
    assert(got->type == SOLID_STYLE_VTYPE_INT);
    assert(got->as.int_val == 320);

    /* Overwrite */
    v = solid_style_int(640);
    solid_style_map_set(&smap, SOLID_STYLE_PROP_WIDTH, v);
    got = solid_style_map_get(&smap, SOLID_STYLE_PROP_WIDTH);
    assert(got != NULL);
...
static void test_value_constructors(void)
{
    ...
    SolidStyleValue vs = solid_style_string("hello");
    assert(vs.type == SOLID_STYLE_VTYPE_STRING);
    assert(strcmp(vs.as.string_val, "hello") == 0);

    SolidStyleValue vb = solid_style_bool(1);
...
static void test_style_apply_runs(void)
{
     ...
     * Just verifying that solid_style_apply does not crash.
     */
    SolidStyleMap smap;
    solid_style_map_init(&smap);

    solid_style_map_set(&smap, SOLID_STYLE_PROP_WIDTH,  solid_style_int(200));
    solid_style_map_set(&smap, SOLID_STYLE_PROP_HEIGHT, solid_style_int(100));
    solid_style_map_set(&smap, SOLID_STYLE_PROP_COLOR,
                        solid_style_color(0xFF, 0x00, 0x00, 0xFF));

    /* This should iterate the map and call the apply functions */
    solid_style_apply(&smap, NULL);
...
static void test_multiple_properties(void)
{
    ...
    solid_style_map_set(&smap, SOLID_STYLE_PROP_WIDTH,  solid_style_int(100));
    solid_style_map_set(&smap, SOLID_STYLE_PROP_HEIGHT, solid_style_int(200));
    solid_style_map_set(&smap, SOLID_STYLE_PROP_COLOR,
                        solid_style_color(0, 0, 0xFF, 0xFF));
    solid_style_map_set(&smap, SOLID_STYLE_PROP_BACKGROUND_COLOR,
                        solid_style_color(0xFF, 0xFF, 0xFF, 0xFF));

    assert(smap.count == 4);

    const SolidStyleValue *w = solid_style_map_get(&smap, SOLID_STYLE_PROP_WIDTH);
    const SolidStyleValue *h = solid_style_map_get(&smap, SOLID_STYLE_PROP_HEIGHT);
