...
/* Standard AES S-box and inverse S-box (FIPS-197). */
...
void aes128_init(Aes128Ctx *ctx, const uint8_t key[16]) {
    ...
    for (int r = 1; r <= 10; r++) {
        uint8_t *prev = ctx->round_keys[r - 1];
        uint8_t *cur  = ctx->round_keys[r];
        uint8_t t[4];
        /* RotWord(SubWord(prev[12..15])) xor Rcon */
        t[0] = (uint8_t)(SBOX[prev[13]] ^ RCON[r]);
        t[1] = SBOX[prev[14]];
        t[2] = SBOX[prev[15]];
        t[3] = SBOX[prev[12]];
...
/* Column-major layout: s[col*4 + row]. */
...
void aes128_encrypt_block(const Aes128Ctx *ctx,
    ...
    for (int r = 1; r < 10; r++) {
        ...
        add_round_key(s, ctx->round_keys[r]);
    }
    sub_bytes(s); shift_rows(s);
    add_round_key(s, ctx->round_keys[10]);
    memcpy(out, s, 16);
}
...
void aes128_cbc_encrypt(const uint8_t key[16], const uint8_t iv[16],
    ...
    Aes128Ctx ctx; aes128_init(&ctx, key);
...
void aes128_cbc_decrypt(const uint8_t key[16], const uint8_t iv[16],
    ...
    for (size_t off = 0; off < len; off += 16) {
        ...
        /* out may alias in, so read in[off] before writing out[off]. */
        uint8_t save[16]; memcpy(save, in + off, 16);
